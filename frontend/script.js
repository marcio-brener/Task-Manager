...
// ==========================
const API_URL = "http://192.168.0.17:5000/api/tasks";
...
function createTask(task) {
  ...
  card.dataset.done = task.done.toString();
  ...
  card.innerHTML = `
    <div class="d-flex justify-content-between align-items-start mb-2">
      ...
      <div class="dropdown">
        <button class="btn btn-sm btn-dark border-0" type="button" data-bs-toggle="dropdown">
          ⋮
        </button>
        <ul class="dropdown-menu dropdown-menu-dark">
          <li>
            <button class="dropdown-item" onclick="openEditModal(this.closest('.task-item'))">
              Editar
            </button>
          </li>
          <li>
            <button class="dropdown-item text-danger" onclick="removeTask(this.closest('.task-item'))">
              Remover
            </button>
          </li>
        </ul>
    ...
    <div class="task-status">
      ${task.done ? finalizada() : getDeadlineStatus(task.deadline)}
    </div>
...
async function toggleTask(card, done) {
  ...
  try {
    ...
    await fetch(`${API_URL}/${id}`, {
      ...
      body: JSON.stringify({
        text: card.querySelector(".task-title").innerText,
    ...
    card.dataset.done = done.toString();
    const title = card.querySelector(".task-title");
    const status = card.querySelector(".task-status");

    if (done) {
      title.classList.add("completed");
      status.innerHTML = finalizada();
    } else {
      title.classList.remove("completed");
      status.innerHTML = getDeadlineStatus(card.dataset.deadline);
    }
...
function sortTasks() {
  ...
  cards.sort((a, b) => order[a.dataset.priority] - order[b.dataset.priority]);
...
function updateCounters() {
  ...
  const done = document.querySelectorAll(".task-item[data-done='true']");

  document.getElementById("totalTasks").innerText = tasks.length;
  document.getElementById("doneTasks").innerText = done.length;

  const percent = tasks.length ? (done.length / tasks.length) * 100 : 0;
...
function getDeadlineStatus(date) {
  ...
  if (date < today) {
    return `<span class="text-danger fw-semibold">Atrasada</span>`;
  }
  if (date === today) {
    return `<span class="text-warning fw-semibold">Hoje</span>`;
  }
  return `<span class="text-success fw-semibold">No prazo</span>`;
}
...
// ==========================
function finalizada() {
  return `<span class="text-primary fw-semibold">Finalizada</span>`;
}